...
type OrganizationSettingsTab = 'general' | 'email';
...
export class OrganizationSettingsComponent {
  ...
  private readonly customEmailPermitted = computed(() => this.auth.isVendor() && this.auth.hasRole('admin'));

  protected readonly customEmailVisible = computed(
    () => this.customEmailPermitted() && this.featureFlags.isCustomEmailsEnabled()
  );
