...
- CNAME pre-verification (resolve domain → expected target) for better UX; the `ask` gate keeps it safe either way. **Landed later** as `internal/dns.VerifyCNAME`, live and unpersisted rather than a cached/background-checked status, and requested per domain by its own endpoint so the lookup never delays the domain list.
