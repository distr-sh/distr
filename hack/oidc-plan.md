...
> **Status: implemented and e2e-validated.** All four PR scopes below landed in a single change: `CustomDomain` table (migration 114) + CRUD API + validation, the internal caddy-ask server (`INTERNAL_SERVER_ADDR`), the Helm chart Caddy StatefulSet with `distr-internal-caddy-ask` Service, and the org settings "Custom Domains" UI with CNAME instructions. Legacy `OrganizationBranding` domain columns remain supported as fallback (migration is the follow-up ticket 0). CNAME pre-verification and the host-context middleware were not included.
>
> **Caddy runs as a StatefulSet** (default 2 replicas) with a per-pod `volumeClaimTemplate` for `/data` instead of a Deployment. Replicas do not share certificate storage, so each one issues its own certificate per domain — accepted deliberately, since sharing would require a `ReadWriteMany` volume or a custom Caddy image with a clustered storage module (`caddy.storage` exposes that path for operators who want it).
>
> **E2E validated on minikube** (values in `hack/minikube-custom-domains-values.yaml`, Caddy's internal CA as issuer): feature gating (403 without `custom_domains`), platform-domain/hostname/duplicate rejection (400/409), domain normalization on create and on `ask`, `ask` returning 200/404/400 and being absent from the public app and registry servers, on-demand TLS issuance for a registered domain, TLS handshake failure for an unregistered one, `/v2/` routed to the registry (`Basic realm="Distr"`) and everything else to the app, HTTP→HTTPS redirect, portal branding resolved by custom domain (204 on the default host), `registryHost` falling back app domain → instance default after deleting the registry domain, per-pod certificate stores surviving a pod restart, and Caddyfile changes rolling the StatefulSet via the checksum annotation.
