...
- **Never cache providers.** `oidc.ProviderForConfiguration` builds one per request from the configuration row, so an edit takes effect immediately. Building one costs a single discovery request, because `oidc.Discover` parses the document itself and hands it to `oidc.ProviderConfig` instead of letting `go-oidc` fetch it again.
- A provider is addressed by the **organization slug plus its own slug** (`oidc.CustomLoginPath`), never by its id: the callback URL has to be registered as a redirect URI at the identity provider, so an administrator reads, types and recognizes it. Configuring a provider therefore requires the organization to have a slug. Slugs are validated with `validation.ValidateSlug` (frontend: `util/slug.ts`), the one slug format the product uses.
...
- Issuer URLs are administrator-controlled input that the Hub fetches, so anything that resolves one must go through `oidc.ParseIssuerURL` and `oidc.RestrictedClientContext`, which routes discovery, JWKS, token exchange and userinfo through the client that refuses non-public addresses. The endpoints of a discovery document are administrator-controlled as well, so it is not enough to guard the discovery request alone. An issuer on http or a non-public address is only accepted when the instance itself is not served over https (`nonPublicIssuersAllowed`, i.e. an `http` `DISTR_HOST`), which is how local development works — there is deliberately no env var for it, and an operator with an identity provider inside their own network configures it as the instance-wide generic provider.
- Every flow, instance-configured or organization-configured, goes through `identityFromIDToken`: one place verifies the ID token, compares the nonce of the authorization request and falls back to userinfo for a missing email. Providers that carry no ID token (GitHub) are the only ones with their own extractor, and they get no nonce.
...
- Exclusivity and membership are checked on **every** login (`checkCustomOIDCLoginAllowed`), never only when the identity is created, so an identity that outlives its membership grants no access. For the same reason an invite is only refused when the account belongs to _another_ organization's provider: a removed member has to be invitable again without an administrator deleting their identity first.
- Removing a member deletes their identities of that organization's providers, next to the access tokens and tutorial progress `deleteUserAccountHandler` already deletes. This is an explicit delete rather than a cascade from the membership row: the identity would have to carry a denormalized `organization_id` to hang a foreign key on, and a cascade would fire invisibly for anything that ever rewrites an assignment row instead of updating it.
