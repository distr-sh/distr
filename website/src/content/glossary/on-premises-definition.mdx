...
alternateNames:
  - On-Prem
  - On-Premise
shortDefinition: 'On-premises (or on-prem) describes software and IT infrastructure that organizations run inside environments they control. The organization controls the environment (traditionally their hardware; today often their cloud account), sets the security policy, and owns the data. Who operates the application can vary.'
lastUpdated: 2026-08-20
faq:
  - question: What does on-premise deployment mean?
    answer: "It means software runs on infrastructure controlled by the customer. That can be physical data centers or their own cloud accounts. The customer controls the environment instead of relying on the vendor's multi-tenant SaaS platform. Application operation may still be customer-led, shared, or vendor-assisted."
  ...
  - question: What is the difference between on-premise and cloud?
    answer: 'On-premise, the customer controls the infrastructure. Cloud (multi-tenant SaaS), the vendor operates the software in their environment. The difference is control. On-premise customers control their deployment environment. SaaS customers access a shared service.'
  ...
  - question: Can on-premises software work without internet?
    answer: 'It can be designed to. On-prem is not air-gapped by default. A true air-gapped deployment is a separate, stricter posture with offline packaging and no vendor access.'
  - question: What is the difference between on-premises and self-hosted?
    answer: 'Related, not identical. On-prem usually means the customer controls the environment (middle of the spectrum). Self-hosted is the umbrella for anything in the customer trust boundary, including BYOC and air-gapped. See the self-hosted spectrum.'
  - question: How do software vendors distribute on-premises software?
    answer: 'Through software distribution platforms (Distr, for example) that distribute containers, automate artifact distribution, track versions across customer installations, enforce licenses, and optionally collect telemetry. This is what lets vendors support both SaaS and [self-hosted deployment](/glossary/self-hosted-software/) efficiently.'
---

## What does on-premises mean?

**On-premises** (often shortened to **on-prem**) describes software and IT infrastructure that organizations run inside environments they control. The organization controls the environment (traditionally their hardware; today often their cloud account), sets the security policy, and owns the data. The vendor does not own that environment. Application lifecycle may still be customer-led, shared, or vendor-assisted.

On-premises today covers two setups. The traditional case is physical servers and infrastructure inside the organization's own data centers or facilities. The modern case is infrastructure inside the organization's own cloud accounts: AWS VPCs, Azure subscriptions, GCP projects. Either way, the distinguishing factor is control. On-premises means the customer controls the deployment environment. [SaaS](/glossary/saas-definition/) means the vendor does.

On the [self-hosted spectrum](/blog/self-hosted-spectrum/), on-prem / VPC / private cloud usually sit in the middle: customer-controlled environment, with application operation negotiated deal by deal.
...
- Distribution, via [container registries](/glossary/oci-container-artifact-registry/), download portals, or artifact repositories
- Installation, via Docker Compose files, [Helm charts](/glossary/helm-chart/), installers, or [Kubernetes](/glossary/kubernetes/) operators
- Updates across thousands of independent customer environments
- [Licensing](/glossary/software-license-management/) that works offline
- Support and remote diagnostics without direct environment access

Modern [software distribution platforms](/glossary/software-distribution-platform/) like Distr exist to manage these complexities so vendors can offer on-prem without doubling engineering costs.
...
| **Internet Requirement**     | Can be designed for offline / air-gapped           | Requires internet connectivity                |
...
| **Maintenance**              | Usually customer; may be shared or vendor-assisted | Vendor handles all maintenance                |
...
- **CRM platforms**: Microsoft Dynamics 365 on-premises, SugarCRM (Sugar Enterprise)
...
Many widely used open-source products are built to run on customer infrastructure, often alongside a hosted SaaS edition:

- **Business software (ERP/CRM)**: Odoo (Community and Enterprise editions run on-premises)
- **DevOps platforms**: GitLab Self-Managed
- **Web analytics**: Matomo On-Premise, the self-hosted alternative to Google Analytics
- **Team messaging**: Mattermost, the self-hosted alternative to Slack
- **Software distribution**: Distr, the open-source distribution platform vendors run [self-hosted](/docs/self-hosting/) to ship software to their own on-prem customers
...
Government agencies, defense contractors, financial institutions, and critical-infrastructure operators often run [air-gapped networks](/glossary/air-gapped-meaning/) with limited or no internet connectivity. SaaS cannot reach those environments at all.
...
[Kubernetes](/glossary/kubernetes/) gives you consistent deployment across customer data centers, AWS VPCs, Azure private clusters, or Google GKE. Vendors package applications as Helm charts and deploy anywhere Kubernetes runs. The underlying infrastructure becomes an abstraction layer.

### Self-hosted cloud, the new on-premises

Many "on-premises" deployments today run in customer-managed AWS, Azure, or GCP accounts rather than physical data centers. From the vendor's perspective, it's operationally identical to traditional on-prem: the customer controls the infrastructure, the vendor provides the software. This is [self-hosted software](/glossary/self-hosted-software/) in cloud infrastructure.

A related model is [BYOC (Bring Your Own Cloud)](/glossary/byoc-definition/), where the vendor manages the software inside the customer's cloud account, a hybrid of SaaS convenience and on-prem control.
...
Scale from your first self-hosted customer to thousands without scaling engineering overhead in lockstep.
