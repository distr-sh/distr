...
faq:
  ...
  - question: What is the best registry for software vendors distributing to customers?
    answer: 'Software vendors distributing commercial applications to external customers should use purpose-built registries like [Distr](/docs/registry/), which provide customer-specific access controls, usage analytics, license management, and support for self-hosted and [air-gapped](/glossary/air-gapped-meaning/) customer deployments.'
---
...
- Rate limiting on anonymous pulls (since April 2025, Docker Hub caps unauthenticated users at [10 pulls per hour per IPv4 address](https://docs.docker.com/docker-hub/usage/); free authenticated accounts get 100 per hour)
...
- Software vendors distributing to [self-hosted](/glossary/self-hosted-software/) or [on-premises](/glossary/on-premises-definition/) customer environments
...
**Image signing** uses cryptographic signatures to prove an image's authenticity and detect tampering. Tools like Docker Content Trust, Notary, and [Sigstore Cosign](https://www.sigstore.dev/) handle signing and verification.
...
**5. Self-hosted and on-premises deployments.** Customers keep demanding [self-hosted software](/glossary/self-hosted-software/) running in their own infrastructure ([BYOC, Bring Your Own Cloud](/glossary/byoc-definition/), [on-premises](/glossary/on-premises-definition/), private clouds). Vendors need registry solutions that actually support that.
...
- **Manual image transfer**: physical media (USB drives, external drives) moving images across the gap
...
Make the most of layer caching:
...
**[Helm charts](/glossary/helm-chart/)**, Kubernetes application packages:
...
**[Software Bill of Materials (SBOM)](/glossary/sbom-software-bill-of-materials/)**, security metadata about software components:
...
Beyond caching public images, vendors can also use a pull-through cache to give customers access to private images without exposing the upstream registry. See our guide on [pull-through caches for software distribution](/blog/container-registry-pull-through-cache/) and the [Distr pull-through cache docs](/docs/registry/pull-through-cache/).
...
- **Air-gapped support**: distribute to disconnected environments without internet connectivity
- **Open source**: available as [managed SaaS](/onboarding/) or [self-hosted](/docs/self-hosting/)
