...
faq:
  ...
  - question: How do you update software in air-gapped environments?
    answer: 'A true air-gapped update uses offline or customer-controlled transfer: physical media, controlled transfer stations, one-way diodes or a private staging path the customer operates. Outbound-only access to a vendor registry is a restricted network, not a true air-gapped deployment. Vendors have to bundle all dependencies for offline installation.'
  - question: What is air-gapped cloud?
    answer: 'Often means cloud environments with strong isolation: private VPCs with no internet access, dedicated accounts and strict network controls. That is restricted or air-gapped-style cloud, not always a true air-gapped deployment. A true air-gapped deployment still means no vendor path and no reliable live connection to untrusted networks.'
...
On the [self-hosted spectrum](/blog/self-hosted-spectrum/), air-gapped is the fully customer-operated pole: **no vendor access** and no reliable live path to a vendor SaaS control plane.
...
Organizations rarely choose air-gapped deployment on a whim. It gets implemented because regulatory, security, or operational requirements demand the highest level of protection.
...
The global average cost of a data breach was $4.44 million in 2025, with the US average at a record $10.22 million, according to [IBM's Cost of a Data Breach Report](https://www.ibm.com/reports/data-breach). Numbers like these keep air-gapped security on the priority list despite its operational overhead.
...
Network connectivity exists only insofar as the design still eliminates routes to untrusted networks. Strict software-defined boundaries enforce that isolation:
...
Segmentation alone (VLANs, firewalls, RBAC) without removing untrusted routes is **restricted networking**, not a true air-gapped deployment.
...
**Example**: a financial institution's backup infrastructure in a separate security zone with no path to the public internet.
...
### 4. Restricted / air-gapped-style cloud

Cloud services operating with strong isolation principles. This is adjacent to true air-gapped deployments on the [self-hosted spectrum](/blog/self-hosted-spectrum/), not the same thing:
...
Encrypted tunnels for **vendor** management access make this a restricted private-cloud posture, not a true air-gapped deployment.

**Advantages**: combines cloud scalability with isolation principles.
**Disadvantages**: not truly air-gapped; depends on cloud provider controls and whether any vendor path remains.
...
- **Dedicated sanitized media**: USB drives used exclusively for air-gapped transfers
...
- **Cryptographic signing**: validating file integrity during transfer so tampered files are detected before they reach production
...
Extreme isolation doesn't make air-gapped systems impenetrable. Sophisticated attackers have demonstrated otherwise. [ESET researchers have documented 17 malicious frameworks](https://www.welivesecurity.com/2021/12/01/jumping-air-gap-15-years-nation-state-effort/) built by nation-state actors specifically to breach air-gapped networks.

### Notable air gap breaches

**[Stuxnet](https://en.wikipedia.org/wiki/Stuxnet) (2010)** was the most famous air gap attack. It targeted Iran's nuclear enrichment facilities through infected USB drives, exploited multiple zero-day vulnerabilities, and demonstrated that nation-state actors can breach even the most secure air-gapped systems.
...
- Managed services designed for air-gapped-style protection
...
**Security patches.** Critical [CVE](/glossary/cve-common-vulnerabilities-and-exposures/) fixes require emergency offline distribution.

**License validation.** [License keys](/glossary/software-license-management/) have to work offline, without phoning home.
...
- **No vendor access**: the isolated environment has no vendor path in
...
- Use dedicated, sanitized devices exclusively for air-gapped transfers
...
Distr makes this manageable whether you have one air-gapped customer or hundreds of isolated deployments across government, healthcare, and critical infrastructure.

**Related topics:**
[On-Premises Deployment](/glossary/on-premises-definition/) | [Self-Hosted Software](/glossary/self-hosted-software/) | [ISV Software Distribution](/glossary/isv-meaning/) | [Software Distribution Platform](/glossary/software-distribution-platform/) | [Container Registry](/glossary/oci-container-artifact-registry/)
