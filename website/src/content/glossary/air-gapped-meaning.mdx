...
faq:
  ...
  - question: How do you update software in air-gapped environments?
    answer: 'True air-gap updates use offline or customer-controlled transfer: physical media, controlled transfer stations, one-way diodes, or a private staging path the customer operates. Outbound-only access to a vendor registry is a restricted network, not a true air gap. Vendors have to bundle all dependencies for offline installation.'
  - question: What is air-gapped cloud?
    answer: 'Often means cloud environments with strong isolation: private VPCs with no internet access, dedicated accounts, strict network controls. That is restricted or air-gap-style cloud, not always a true air gap. True air-gap still means no vendor path and no reliable live connection to untrusted networks.'
...
On the [self-hosted spectrum](/blog/self-hosted-spectrum/), air-gap is the fully customer-operated pole: **no vendor access** and no reliable live path to a vendor SaaS control plane.
...
Segmentation alone (VLANs, firewalls, RBAC) without removing untrusted routes is **restricted networking**, not a true air gap.
...
### 4. Restricted / air-gap-style cloud

Cloud services operating with strong isolation principles. This is adjacent to true air-gap on the [self-hosted spectrum](/blog/self-hosted-spectrum/), not the same thing:
...
Encrypted tunnels for **vendor** management access make this a restricted private-cloud posture, not a true air gap.
