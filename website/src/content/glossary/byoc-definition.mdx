...
description: 'Complete guide to BYOC (Bring Your Own Cloud): what it means, how it works, BYOC vs SaaS vs self-hosted, architecture, benefits, challenges, and when to use BYOC for software distribution and data services.'
term: BYOC
alternateNames:
...
faq:
  ...
  - question: Can I use BYOC with on-premises infrastructure?
    answer: 'Traditional BYOC targets public cloud. Similar models exist for [on-premises](/glossary/on-premises-definition/) deployments and are often called "customer data center" or "[self-hosted](/glossary/self-hosted-software/)" offerings. Some vendors support hybrid models that bridge cloud and on-premises.'
  ...
  - question: Does BYOC work with air-gapped environments?
    answer: 'Standard BYOC needs connectivity for vendor control plane access. True [air-gapped](/glossary/air-gapped-meaning/) deployments sit at the other end of the [self-hosted spectrum](/blog/self-hosted-spectrum/): fully customer-operated, offline updates and no vendor access.'
...
BYOC is the vendor-operated end of [self-hosted](/glossary/self-hosted-software/): the app runs in the customer's cloud and you still operate it so it can feel like [SaaS](/glossary/saas-definition/). The contrast is multi-tenant SaaS, where workloads live in your cloud. For how BYOC sits next to customer-operated and [air-gapped](/glossary/air-gapped-meaning/) postures, read [The self-hosted spectrum](/blog/self-hosted-spectrum/).
:::
...
The model caught on as organizations wanted more control over their data and infrastructure while still getting the operational benefits of managed services. BYOC is the answer for teams that need data and workloads in their own cloud account, without taking on full day-2 operations themselves.
...
## BYOC vs multi-tenant SaaS vs customer-operated self-hosted

Multi-tenant [SaaS](/glossary/saas-definition/) keeps workloads in the vendor's environment. [Self-hosted](/glossary/self-hosted-software/) keeps them in the customer's. BYOC is one point on that self-hosted spectrum. Full map: [The self-hosted spectrum](/blog/self-hosted-spectrum/).

**[Multi-tenant SaaS](/glossary/saas-definition/):**
...
- Vendor operates the application in the customer's cloud account; the customer owns and governs that account (provisioning often still vendor-led)
- Shared responsibility: vendor handles the application, customer governs cloud policy and account boundaries
...
**Customer-operated [self-hosted](/glossary/self-hosted-software/):**

- Software runs in customer infrastructure (cloud or [on-premises](/glossary/on-premises-definition/))
- Customer manages installation, configuration, updates, monitoring (or accepts only artifacts for their own pipeline)
...
| Aspect                 | Multi-tenant SaaS | BYOC           | Customer-operated |
| ---------------------- | ----------------- | -------------- | ----------------- |
...
5. **[License management](/glossary/software-license-management/).** [Entitlement](/glossary/software-entitlement/) checks that run in the customer environment and report back to the vendor control plane.
