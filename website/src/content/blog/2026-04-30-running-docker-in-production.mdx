...
description: 'Yes, plain Docker Compose can still run production workloads in 2026, but only if you close the operational gaps it leaves: cleanup, healing, image pinning, socket security, and updates.'
...
I am Philip, an engineer working at Distr, which helps software and AI companies distribute their applications to self-hosted environments.
...
The Distr Docker agent does mount the socket. It has to, in order to orchestrate Compose and Swarm on the host. We document that boundary openly in the [Docker agent docs](/docs/agents/docker-agent) so customer security teams can review it before installation. The agent authenticates to Distr with a JWT, and the install secret is shown once and never stored.
