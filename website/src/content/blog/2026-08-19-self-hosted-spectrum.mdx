---
title: 'The self-hosted spectrum: from BYOC to air-gap'
description: 'Self-hosted is the opposite of multi-tenant SaaS. Inside that, there is still a spectrum: BYOC on one end, air-gap on the other.'
...
**[Multi-tenant SaaS](/glossary/saas-definition/)** means you run one cloud service that serves many customers. Their data and workloads live in _your_ environment. You own uptime, upgrades, and most of the security boundary. Customers get speed and less ops, and give up where the data lives.
...
You will not SSH into the cluster. Debugging often means **[support bundles](/blog/support-bundles/)** the customer generates and sends out (logs, versions, config redacted to their policy), then you diagnose offline and send a fix back the same way. If you say yes to air-gap, you are signing up for release logistics and support without live access, not for a slightly stricter VPC checkbox.
...
Air-gap is still self-hosted. It is just the end of the spectrum where managed-SaaS habits stop working.

## The middle: where most deals actually live

Most conversations are not pure BYOC and not true air-gap. The customer wants the product in **their VPC** or **[on their hardware](/docs/use-cases/assisted-self-managed/)**, and then you invent a distribution stack.
...
- Some customers allow **outbound logs**; others are closer to air-gap and need a one-shot **[support bundle](/blog/support-bundles/)**
...
The trap is treating each point as a different product: a BYOC fork, an on-prem fork, an air-gap fork.
...
**Previous:** [Why more and more B2B startups are shipping self-hosted](/blog/why-b2b-startups-shipping-self-hosted/)
