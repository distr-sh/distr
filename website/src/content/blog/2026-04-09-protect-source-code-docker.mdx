...
- Use image digests (`image: myapp@sha256:abc123...`) instead of mutable tags for integrity verification. The same point comes up in [Running Docker in Production](/blog/running-docker-in-production/) under why `:latest` is a lie, with the commands for retrieving and pinning a digest.
