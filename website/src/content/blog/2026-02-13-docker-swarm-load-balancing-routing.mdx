...
You need SSL certificates, host-based routing, path-based routing, and rate limiting, none of which the built-in routing mesh provides.
...
[Distr](https://distr.sh) can orchestrate Docker Swarm clusters alongside Docker Compose deployments, handling the distribution of stack files to self-hosted customer environments.
...
Its networking model (overlay networks, DNS-based service discovery, and the routing mesh) handles the hard parts of distributed networking, so you can focus on your application logic.
