...
<Aside type="caution">
  `STATUS_ENTRIES_MAX_AGE`, `METRICS_ENTRIES_MAX_AGE` and `LOG_RECORD_ENTRIES_MAX_COUNT` do not have a default value.
  If unset, the cleanup routine for these targets will have no effect.
</Aside>
