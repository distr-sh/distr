...
rotation some values are still encrypted with the previous key. This job moves both to the active key:
...
cronJobs:
  - name: database-encryption
    labels:
      distr.sh/job: database-encryption
    args: [maintenance, encrypt-database, --timeout, 60m]
```
