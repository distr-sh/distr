...
description: Complete reference of all environment variables used to configure a self-hosted Distr instance, grouped by area.
...
This page lists all environment variables used to configure a self-hosted Distr instance.
Variables marked as **required** must be set for Distr to start (some are only required when a related feature is enabled).
...
| `JWT_SECRET`                          | yes      | —         | Base64-encoded secret every token is signed with, see [Security & Encryption](/docs/self-hosting/security#jwt-signing-secret). Generate with `openssl rand -base64 32`. |
| `DATABASE_ENCRYPTION_KEY`             | yes      | —         | Base64-encoded key that sensitive database columns are encrypted with, see [Security & Encryption](/docs/self-hosting/security#database-encryption).                    |
| `DATABASE_ENCRYPTION_MIGRATE_ON_BOOT` | no       | `false`   | Encrypt values that are still stored in plaintext during startup. Single instance deployments only.                                                                     |
...
| `BLOCKED_EMAIL_DOMAINS`               | no       | —         | Comma-separated email domains that cannot sign up or log in, see [Blocklists](#blocklists).                                                                             |
| `BLOCKED_IPS`                         | no       | —         | Comma-separated IP addresses and CIDR ranges that cannot sign up or log in, see [Blocklists](#blocklists).                                                              |
...
| `SUPPORT_EMAIL`                       | no       | —         | Address users can contact for help, offered as a `mailto:` link where the frontend mentions support. Unset means support is mentioned without a link.                   |
...
`BLOCKED_EMAIL_DOMAINS` (e.g. `example.com,spam.io`) refuses registration, password login and login through the
built-in OIDC providers for addresses of a listed domain or any of its subdomains, so `example.com` also covers
`mail.example.com`. Matching ignores case. Organizations' own OIDC providers are not affected.

`BLOCKED_IPS` (e.g. `203.0.113.7,198.51.100.0/24,2001:db8::/32`) refuses every request to the sign-up, login and
password reset endpoints from a listed address or range. When a request carries an `X-Forwarded-For` header, the
client address is its rightmost entry, so it is only accurate with exactly one reverse proxy in front of Distr.
A request whose address cannot be determined is not blocked.

Both answer with `403 Forbidden`, except a blocked email domain on a built-in OIDC login, which redirects back to the
login page and shows the reason there.

### Maintenance mode

`MAINTENANCE_MODE=true` announces planned downtime instead of stopping the instance. Distr keeps serving the
...
Distr unwraps `JWT_SECRET`, `DATABASE_ENCRYPTION_KEY` and `LICENSE_KEY_PRIVATE_KEY` with AWS KMS or GCP Cloud KMS
during startup, so that no key material is kept in your configuration.
See [Security & Encryption](/docs/self-hosting/security#key-management-service) for details.
...
| Variable              | Required | Default | Description                                                                                                                                                                                                                                                                                 |
| --------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AGENT_HOST`          | no       | —       | Host (and optional port, no scheme) agents and connect commands reach this instance at, taking precedence over an organization's custom app domain. Its scheme is taken from `DISTR_HOST`. Unset, agents use the organization's custom app domain if it has one and `DISTR_HOST` otherwise. |
| `AGENT_INTERVAL`      | no       | `5s`    | Interval at which agents poll Distr for updates.                                                                                                                                                                                                                                            |
...
These limit what a single client IP address may pull from [public
artifacts](/docs/registry/#public-artifacts) without credentials. Authenticated pulls are not rate
limited. A value of `0` disables the limit it stands for. The limits are counted per Distr replica
and in memory, so they reset on a rollout and a client spread across replicas may exceed them by up
to a factor of the replica count.

| Variable                                            | Required | Default | Description                                                  |
| --------------------------------------------------- | -------- | ------- | ------------------------------------------------------------ |
| `REGISTRY_ANONYMOUS_MANIFEST_RATE_LIMIT_PER_MINUTE` | no       | `10`    | Anonymous manifest and tag list requests per IP, per minute. |
| `REGISTRY_ANONYMOUS_MANIFEST_RATE_LIMIT_PER_HOUR`   | no       | `30`    | Anonymous manifest and tag list requests per IP, per hour.   |
| `REGISTRY_ANONYMOUS_BLOB_RATE_LIMIT_PER_MINUTE`     | no       | `60`    | Anonymous blob requests per IP, per minute.                  |
| `REGISTRY_ANONYMOUS_BLOB_RATE_LIMIT_PER_HOUR`       | no       | `300`   | Anonymous blob requests per IP, per hour.                    |

## Custom Domains

Custom domain self-service lets vendor organizations serve Distr under their own domains.
It requires an edge proxy with on-demand TLS (e.g. Caddy, included in the Helm chart) in front of Distr.
...
| `CUSTOM_DOMAIN_VERIFICATION_CRON`          | no       | —       | Cron schedule of the built-in domain verification job. If unset, no verification is scheduled in Distr.                          |
...
| `CLEANUP_USER_ACCOUNT_CRON`                 | no       | —       | Cron schedule for deleting user accounts that belong to no organization.    |
| `CLEANUP_USER_ACCOUNT_TIMEOUT`              | no       | `0`     | Timeout for the user account cleanup run.                                   |
| `CLEANUP_USER_ACCOUNT_MIN_AGE`              | no       | `720h`  | Time since the last login, or creation if never logged in, before deletion. |
| `CLEANUP_FILE_CRON`                         | no       | —       | Cron schedule for deleting files that nothing references any more.          |
| `CLEANUP_FILE_TIMEOUT`                      | no       | `0`     | Timeout for the file cleanup run.                                           |
| `CLEANUP_FILE_MIN_AGE`                      | no       | `24h`   | Minimum age of an unreferenced file before it is deleted.                   |
...
instance configuration. Those issuer URLs are entered by organization administrators and fetched by Distr, so they
...
| `LICENSE_KEY_PRIVATE_KEY`                  | no       | —       | PEM-encoded private key used to sign customer license keys, see [Security & Encryption](/docs/self-hosting/security#license-signing-key). |
