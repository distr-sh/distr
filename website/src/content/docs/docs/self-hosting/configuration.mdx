...
The feature is only offered in the UI when `CUSTOM_DOMAIN_APP_CNAME_TARGET` is set.

| Variable                              | Required | Default | Description                                                                                                               |
| ------------------------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------------- |
| `CUSTOM_DOMAIN_APP_CNAME_TARGET`      | no       | —       | Hostname customers point their app domain CNAME records at (e.g. `cname.distr.sh`). Enables custom domain self-service.   |
| `CUSTOM_DOMAIN_REGISTRY_CNAME_TARGET` | no       | —       | Hostname customers point their registry domain CNAME records at. Falls back to the app CNAME target if unset.             |
...
| `CUSTOM_OIDC_ALLOW_PRIVATE_ISSUERS`             | no                 | `false` | Allows `http` and private network issuers for organization-configured providers.  |

Vendor organizations on the Business plan can additionally configure their own providers in the UI, which requires no
instance configuration. Those issuer URLs are entered by organization administrators and fetched by the Hub, so they
must be `https` and must not resolve to a private network address. `CUSTOM_OIDC_ALLOW_PRIVATE_ISSUERS` lifts both
restrictions and must only be set for local development or an instance whose identity providers run on an internal
network.
