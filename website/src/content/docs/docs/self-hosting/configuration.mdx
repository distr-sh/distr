...
resources, MFA secrets, Stripe webhook secrets and entitlement registry passwords. Personal access tokens are stored
as a keyed hash instead, since they are only ever looked up and never read back.
...
Personal access tokens are the exception: they are hashed rather than encrypted, so they cannot be moved to a new key.
They keep working for as long as the key they were created under stays in the list, and have to be reissued to retire
it.
