...
One of each runs everything on a single VM or cluster, and the ones for
[AWS](/docs/self-hosting/docker/#distr-enterprise-on-aws) and
[GCP](/docs/self-hosting/docker/#distr-enterprise-on-gcp) run the Hub against a managed database and
managed object storage instead, either with
[Docker Compose](/docs/self-hosting/docker/#running-in-production) or on
[Kubernetes](/docs/self-hosting/kubernetes/#running-in-production).
