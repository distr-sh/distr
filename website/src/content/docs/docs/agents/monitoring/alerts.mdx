...
<Aside type="note">
  Resource usage alerts require [metrics collection](/docs/agents/logs-and-metrics/#metrics) to be enabled on the deployment target. If metrics are not enabled, resource triggers will not fire.
</Aside>

<Aside type="note">
  Disk usage alerts are only available for Docker agent deployment targets. The alert email includes the device name and mount path that exceeded the threshold.
</Aside>
