...
This step-by-step guide shows you how to create deployments for Docker and Helm applications in Distr. Before creating a deployment, make sure you've already [created an application](/docs/agents/application/) with at least one version. The deployment flow guides you through selecting a customer, choosing an application, configuring deployment settings, and obtaining the command to install both the Distr agent and your application.
...
The first step is to select the customer organization that will receive this deployment. If you haven't created any customers yet, you'll need to do that first. See [Onboarding a New Customer](/docs/platform/rbac/#inviting-customer-organization-users) for instructions.
...
<Aside type="tip">
  ...
  an [application entitlement](/docs/platform/application-entitlements/) and
...
After running the command, you can check the deployment status in the **Deployments** section. Click on the status label to see the latest status of each revision, or on the **Inspect** button to view the [logs](/docs/agents/logs-and-metrics/#log-viewer) of the deployment.
...
You can also enable cluster-scoped permissions. When enabled, the agent will have cluster-wide access; otherwise, it will be limited to a specific namespace. See [Kubernetes Agent](/docs/agents/kubernetes-agent/) for more details about cluster-scoped vs namespace-scoped agents.
...
A deployment can follow its application's latest version instead of being updated by hand or by a release pipeline. Distr then creates a new revision for it whenever the latest version changes, which is when a version is created, when an archived version is brought back, when the application's [versioning strategy](/docs/agents/application/#versioning-strategy) changes and when an entitlement starts covering a newer version.

Open the actions menu on a deployment and select **Enable automatic updates**, or tick **Enable automatic updates** while creating or updating the deployment. Both a vendor and a customer can do this. Enabling it deploys the latest version right away, which is also why the version cannot be chosen while it is on.

A deployment is only ever moved forward. Archiving a version or narrowing an entitlement does not roll a deployment back to an older one, and neither does a version that the strategy orders below the one currently deployed.

A deployment whose values no longer apply to the new version is left on the version it runs, rather than being updated into a failing state. The same happens when the deployment is not entitled to the newer version.

The option requires **Allow automatic updates** on the application and is part of the Business plan.
...
- [Application Entitlements](/docs/platform/application-entitlements/) - Control which applications customers can access
- [Docker Environment Variables](/docs/agents/docker-env/) - Configure environment variable templates
- [Helm Chart Registry Auth](/docs/agents/helm-registry-auth/) - Set up authenticated image pulls
