...
Distr can run on a variety of operating systems.
Most enterprise environments run on Linux-based VMs, but there are exceptions.
For example, you might choose macOS as the target for running Distr agents during testing.
...
But if you're setting up Distr on macOS, follow these steps.
...
**Alternative: configure the endpoint on the deployment target**

Instead of switching contexts, you can tell the agent which socket to use. Check **Use a custom Docker endpoint** on the deployment target and enter the endpoint of the context you want to use, for example `unix:///Users/you/.docker/run/docker.sock` for `desktop-linux`. See [Custom Docker Endpoint](/docs/agents/docker-agent/#custom-docker-endpoint).

<Aside type="tip">
  This is also how you point the agent at a Docker-compatible runtime that has no
  `/var/run/docker.sock` at all. If you run [Apple
  Container](https://github.com/apple/container) with
  [socktainer](https://github.com/socktainer/socktainer), enter socktainer's socket
  as the endpoint, for example `unix:///Users/you/.socktainer/container.sock`.
</Aside>
...
After these steps, your macOS environment should be ready to run Distr agents. If you run into problems, [get help](https://github.com/distr-sh/distr/discussions) in our community forum or email us at support@distr.sh.
