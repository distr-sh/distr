...
<Aside type="note">
  Docker's `.env` file syntax does not support multi-line values. If your
  secret contains newline characters, Distr escapes them automatically to `\n`.
  Docker Compose only interprets these escape sequences in double-quoted values,
  so always wrap your secret references in double quotes as shown above.
</Aside>
