...
<Aside type="note">
  Pulls are recorded for all authenticated users — both vendor team members and end customers.
  If a pull is performed by a customer user, the customer organization name is shown.
  Vendor-internal pulls show the user name without a customer organization.
</Aside>
...
<Aside type="tip">
  Filter selections are synced to the URL query parameters.
  You can bookmark or share a filtered view by copying the URL.
</Aside>
...
<Aside type="note">
  ...
  Community and Starter plans include up to 100 rows per export.
  Pro and Trial plans include up to 10,000 rows.
  Enterprise limits are configured per license.
</Aside>
