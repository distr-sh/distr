...
description: Configure Docker Compose secrets and manage sensitive configuration values centrally in Distr for secure, templated deployments.
...
In this guide, you'll learn how to create, manage, and use secrets in your deployment configurations — including how to pass them securely via Docker Compose secrets.
...
If your application requires access to sensitive information without exposing it via environment variables, you can use Docker's native secrets mechanism. This mounts secret values as files inside the container rather than passing them as environment variables.

To configure secrets in Docker Compose, use the `secrets` section of the `compose.yaml` file:
...
You can combine this with Distr secrets by referencing them in your `.env` file:
...
<Aside type="note">
  The `.env` file syntax used by Docker does not support values spanning
  multiple lines. If your secret contains newline characters, Distr escapes them
  automatically. Be aware that Docker Compose only supports such shell escape
  sequences in double-quoted values.
</Aside>

For more details about `.env` files in Docker Compose, see our guide on [Docker Environment Variables](/docs/guides/docker-env/).
