...
## True air-gap: Kubernetes with Zarf (recommended)
...
- **Exchange / staging server:** download to a customer-controlled DMZ or jump host, then move through their approved transfer into the air gap.
- **Data diode / one-way:** same idea: content only moves inbound under their controls.

## Restricted outbound to Distr (not a true air gap)
...
Prefer the true air-gap paths above when there is no outbound path at all. Use restricted pulls when the buyer allows Distr but nothing else.
