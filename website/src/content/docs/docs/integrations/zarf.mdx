...
4. Your customers deploy directly from the registry with `zarf package deploy oci://...`, or download the package once and carry it across the gap
...
For environments without any internet access, customers pull the package on a machine that can reach the Distr registry, transfer it across the gap (for example on physical media), and deploy from the local file:

```shell
# On a machine with registry access
zarf package pull oci://registry.distr.sh/your-slug/hello-distr:1.0.0

# Transfer zarf-package-hello-distr-*.tar.zst across the gap, then:
zarf init --confirm
