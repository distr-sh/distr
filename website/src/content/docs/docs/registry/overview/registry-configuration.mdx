...
slug: docs/registry/configuration
sidebar:
...
import lightSlug from '~/assets/docs/registry/overview/light-slug.png';
import darkSlug from '~/assets/docs/registry/overview/dark-slug.png';
...
import customerArtifactViewLight from '~/assets/docs/registry/overview/customer-artifact-view-light.png';
import customerArtifactViewDark from '~/assets/docs/registry/overview/customer-artifact-view-dark.png';
...
- By default, the registry uses immutable tags, so once pushed, a tag cannot be changed. Immutable tags are a safer and more secure default. If you need to overwrite existing tags, mutable tags can be enabled on the organization settings page ("Artifact versions are mutable").
...
- The [customer portal](/docs/platform/customer-portal/registry/#registry-quickstart-from-the-customer-portal) provides a simplified view, the registry UI only shows the artifacts and versions that are accessible to the user.
