---
title: Artifact Download Analytics
...
import downloadLight from '~/assets/docs/registry/overview/download-light.png';
import downloadDark from '~/assets/docs/registry/overview/download-dark.png';
...
| **User**       | The name or email of the authenticated user, or `Anonymous`            |
| **Deployment** | The deployment target whose agent performed the pull                   |
...
<Aside type="note">
  ...
  customer organization. Agents authenticate with their own token rather than as
  a user, so their pulls have no user and name the deployment target they run on
  instead; the customer organization is taken from that deployment target. A
  pull from a [public artifact](/docs/registry/#public-artifacts) that came
  without credentials is marked **Anonymous** and has neither a user nor a
  customer organization, only an IP address.
</Aside>
...
- **User**: Select a specific user account, or **Anonymous** for the pulls that came without credentials
- **Deployment**: Select a specific deployment target whose agent pulled artifacts
...
| Deployment Target | Deployment target name        |
...
<Aside type="note">
  ...
  The Community Edition includes up to 100 rows per export. Pro, Business, and
  Trial plans include up to 10,000 rows. Enterprise limits are configured per
  license.
</Aside>
...
- **Detect unusual activity**: Filter by IP address to spot pulls from unexpected locations, including anonymous pulls from public artifacts
