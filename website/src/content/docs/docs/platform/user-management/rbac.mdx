...
<Aside type="note">
  Role-based access control is available in all paid plans (Pro, Business, and
  Enterprise) as well as the Pro Trial. The free Community Edition only allows
  one user per customer who will automatically have Administrator privileges.
  All internal users have Administrator privileges as well.
</Aside>
...
<Aside type="note">
  In the Community Edition, all users are automatically assigned the
  Administrator role, and the role selection dropdown is not available.
</Aside>
...
1. Log in to the [Vendor Portal](https://app.distr.sh)
2. Navigate to the **Customers** section in the sidebar (see [Customer Management](/docs/platform/customer-management/) for the full Customers UI)
...
<Aside type="caution">
  If you upgrade to a paid plan, existing users will automatically be assigned
  the Administrator role. You can then change their roles as needed.
</Aside>
...
- Login with Google, GitHub & Microsoft; the Business plan adds enterprise SSO (bring-your-own OIDC)
...
- Advanced identity provider integrations, including SAML SSO
