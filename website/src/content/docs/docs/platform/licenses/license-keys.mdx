...
Each license key has a **Reference** column in the license keys list that shows the exact template variable to use:
...
```dotenv
LICENSE_TOKEN={{ .LicenseKeys.my-license-key }}
```
...
```yaml
app:
  licenseToken: {{.LicenseKeys.my-license-key}}
```
