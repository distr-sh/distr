...
<Aside type="note">
  License Management is a **Pro feature** available in Pro, Business,
  Enterprise, and Pro Trial plans. The free Community Edition does not include
  license management capabilities. Learn more about [subscription
...
Your application verifies the token using the corresponding public key, with no call back to Distr required. This makes verification fast and lets it work in air-gapped or offline environments.
...
<Aside type="note">
  **Payload and dates are immutable after creation.** Once a license key is
  ...
  dates, Distr creates a new license key, and the original remains unchanged.
  You can view all previously generated license keys in the revision history.
</Aside>
