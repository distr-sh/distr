-- IMPORTANT: This migration is not reversible once a support bundle or comment has lost its author.

DROP INDEX fk_UserAccount_image_id;
DROP INDEX fk_Application_image_id;
DROP INDEX fk_Artifact_image_id;
DROP INDEX fk_CustomerOrganization_image_id;
DROP INDEX fk_OrganizationBranding_logo_image_id;
DROP INDEX fk_OrganizationBranding_favicon_image_id;

ALTER TABLE SupportBundle
  ALTER COLUMN created_by_user_account_id SET NOT NULL,
  DROP CONSTRAINT supportbundle_created_by_user_account_id_fkey,
  ADD CONSTRAINT supportbundle_created_by_user_account_id_fkey
    FOREIGN KEY (created_by_user_account_id) REFERENCES UserAccount (id),
  DROP CONSTRAINT supportbundle_status_changed_by_user_account_id_fkey,
  ADD CONSTRAINT supportbundle_status_changed_by_user_account_id_fkey
    FOREIGN KEY (status_changed_by_user_account_id) REFERENCES UserAccount (id);

ALTER TABLE SupportBundleComment
  ALTER COLUMN user_account_id SET NOT NULL,
  DROP CONSTRAINT supportbundlecomment_user_account_id_fkey,
  ADD CONSTRAINT supportbundlecomment_user_account_id_fkey
    FOREIGN KEY (user_account_id) REFERENCES UserAccount (id);
