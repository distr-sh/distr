...
func LockState(ctx context.Context, deploymentID uuid.UUID, lockID, lockInfo string) error {
	...
	cmd, err := db.Exec(ctx,
		...
		WHERE deployment_id = @deploymentID
			AND (lock_id IS NULL OR lock_id = @lockID)`,
		pgx.NamedArgs{
			"deploymentID": deploymentID,
			"lockID":       lockID,
			"lockInfo":     lockInfo,
...
func IsStateLocked(ctx context.Context, deploymentID uuid.UUID) (bool, string, error) {
	db := internalctx.GetDb(ctx)

	var lockID *string
	err := db.QueryRow(ctx,
		`SELECT lock_id FROM opentofu_state WHERE deployment_id = @deploymentID`,
		pgx.NamedArgs{"deploymentID": deploymentID},
	).Scan(&lockID)
	if errors.Is(err, pgx.ErrNoRows) {
		return false, "", nil
	} else if err != nil {
		return false, "", fmt.Errorf("failed to check opentofu_state lock: %w", err)
	}
